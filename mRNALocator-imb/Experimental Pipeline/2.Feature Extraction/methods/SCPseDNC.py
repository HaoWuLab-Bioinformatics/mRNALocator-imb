...
def check_Pse_arguments(method, nctype, weight, kmer, lamadaValue):
    ...
    if not 0 < kmer < 10:
        ...
        sys.exit(1)
    myDictDefault = {
        'SCPseDNC': {'DNA': ['Rise', 'Roll', 'Shift', 'Slide', 'Tilt', 'Twist'], 'RNA': []}
    }
    myDataFile = {
        'SCPseDNC': {'DNA': 'didnaPhyche.data', 'RNA': ''}
    }
...
@njit(cache=True, nogil=True, fastmath=True)
def numba_get_kmer_frequency(seq_arr, kmer, di_index_len):
    seq_len = seq_arr.shape[0]
    kmer_num = seq_len - kmer + 1
    frequency = np.zeros(di_index_len, dtype=np.float64)
    for i in range(kmer_num):
        if kmer == 2:
            idx = 4 * seq_arr[i] + seq_arr[i+1]
            frequency[idx] += 1
    frequency /= kmer_num
    return frequency
    ...
    for tmpLamada in prange(lamadaValue):
        for p in range(prop_len):
            ...
            for i in range(valid_i):
                idx1 = 4 * seq_num_arr[i] + seq_num_arr[i+1]
                idx2 = 4 * seq_num_arr[i + tmpLamada + 1] + seq_num_arr[i + tmpLamada + 1 + 1]
...
def make_SCPseDNC_vector(sequences, myPropertyName, myPropertyValue, lamadaValue, weight, path, dataset_type: str = 'train'):
    encodings = []
    header = ['SampleName']
    di_index_sorted = sorted(myDiIndex.keys())
    for pair in di_index_sorted:
        header.append(pair)
    ...
    di_index_len = len(myDiIndex)
    i = -1
    print(f"making {dataset_type.upper()} SCPseDNC feature (lamada={lamadaValue}, weight={weight})...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        seq_num_arr = dna_str2num_arr(sequence)
        dipeptideFrequency = numba_get_kmer_frequency(seq_num_arr, 2, di_index_len)
        thetaArray = numba_get_theta_array_type2(seq_num_arr, prop_arr, lamadaValue, 2)
        sum_theta = np.sum(thetaArray)
        denom = 1 + weight * sum_theta
        for freq in dipeptideFrequency:
...
method = 'SCPseDNC'
feature_method = make_SCPseDNC_vector
nctype = 'DNA'
weight = 0.1
kmer = 2
lamadaValue = 10
...
print("\nSCPseDNC feature extraction done.")
