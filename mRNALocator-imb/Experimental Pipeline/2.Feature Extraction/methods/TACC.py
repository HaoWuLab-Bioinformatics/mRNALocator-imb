...
def numba_calc_ac(seq_num_arr: np.ndarray, prop_arr: np.ndarray, kmer: int, lag_max: int) -> np.ndarray:
    ...
    for j in range(kmer_num):
        if kmer == 2:
            idx1 = seq_num_arr[j]
            idx2 = seq_num_arr[j+1]
            kmer_idx = 4 * idx1 + idx2
        elif kmer == 3:
            idx1 = seq_num_arr[j]
            idx2 = seq_num_arr[j+1]
            idx3 = seq_num_arr[j+2]
            kmer_idx = 16 * idx1 + 4 * idx2 + idx3
        else:
            kmer_idx = 0
    ...
    for l in prange(1, lag_max + 1):
        ...
        for j in range(valid_j):
            if kmer == 2:
                idx1 = seq_num_arr[j]
                idx2 = seq_num_arr[j+1]
                kmer_idx1 = 4 * idx1 + idx2
                idx1_l = seq_num_arr[j+l]
                idx2_l = seq_num_arr[j+l+1]
                kmer_idx2 = 4 * idx1_l + idx2_l
            elif kmer == 3:
                idx1 = seq_num_arr[j]
                idx2 = seq_num_arr[j+1]
                idx3 = seq_num_arr[j+2]
                kmer_idx1 = 16 * idx1 + 4 * idx2 + idx3
                idx1_l = seq_num_arr[j+l]
                idx2_l = seq_num_arr[j+l+1]
                idx3_l = seq_num_arr[j+l+2]
                kmer_idx2 = 16 * idx1_l + 4 * idx2_l + idx3_l
            else:
                kmer_idx1 = 0
                kmer_idx2 = 0
            acValue += (prop_arr[kmer_idx1] - meanValue) * (prop_arr[kmer_idx2] - meanValue)
        acValue /= valid_j
        ac_vals[l-1] = acValue
    return ac_vals
    ...
    for l in prange(1, lag_max + 1):
        ...
        for j in range(valid_j):
            if kmer == 2:
                ...
                kmer_idx1 = 0
                kmer_idx2 = 0
            ccValue += (prop1_arr[kmer_idx1] - meanP1) * (prop2_arr[kmer_idx2] - meanP2)
        ccValue /= valid_j
        cc_vals[l-1] = ccValue
    return cc_vals
