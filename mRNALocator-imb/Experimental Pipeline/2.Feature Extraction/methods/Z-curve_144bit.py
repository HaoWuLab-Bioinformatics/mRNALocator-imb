...
def numba_calc_zcurve_144bit(seq: str) -> np.ndarray:
    seq_len = len(seq)
    if seq_len < 3:
        return np.zeros(144, dtype=np.float64)
    total_tri = seq_len - 2
    
    pos1_counts = np.zeros(64, dtype=np.int64)
    pos2_counts = np.zeros(64, dtype=np.int64)
    pos3_counts = np.zeros(64, dtype=np.int64)
    ...
    z_vals = np.zeros(144, dtype=np.float64)
    val_idx = 0
    bases = ['A', 'C', 'G', 'T']
    for b1 in bases:
        for b2 in bases:
            ...
            for pos in [1,2,3]:
                counts = pos1_counts if pos ==1 else pos2_counts if pos ==2 else pos3_counts
                
                a_idx = b1_idx + b2_idx + 0
                c_idx = b1_idx + b2_idx + 1
                g_idx = b1_idx + b2_idx + 2
                t_idx = b1_idx + b2_idx + 3
                
                a_count = counts[a_idx]
                c_count = counts[c_idx]
                g_count = counts[g_idx]
                t_count = counts[t_idx]
...
def Z_curve_144bit(sequences, path, dataset_type: str = 'train'):
    NN = 'ACGT'
    encodings = []
    header = ['SampleName']
    ...
    i = -1
    print(f"making {dataset_type.upper()} Z-curve_144bit feature...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        z_vals = numba_calc_zcurve_144bit(sequence)
...
method = 'Z-curve_144bit'
feature_method = Z_curve_144bit
...
print("\nZ-curve_144bit feature extraction done.")
