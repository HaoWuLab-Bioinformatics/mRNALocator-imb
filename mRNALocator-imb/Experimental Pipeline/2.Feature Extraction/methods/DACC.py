...
data_path = os.path.abspath(os.path.dirname(pPath) + os.path.sep + ".") + r'\data' if platform.system() == 'Windows' else os.path.abspath(os.path.dirname(pPath) + os.path.sep + ".") + r'/data'
...
def numba_calc_ac(seq_num_arr, prop_arr, kmer, lag_max):
    ...
    for j in range(kmer_num):
        if kmer == 2:
            ...
            kmer_idx = 16 * seq_num_arr[j] + 4 * seq_num_arr[j+1] + seq_num_arr[j+2]
        else:
            kmer_idx = 0
    ...
    for l in prange(1, lag_max + 1):
        ...
        for j in range(valid_j):
            if kmer == 2:
                ...
                idx2 = 4 * seq_num_arr[j+l] + seq_num_arr[j+l+1]
            elif kmer == 3:
                idx1 = 16 * seq_num_arr[j] + 4 * seq_num_arr[j+1] + seq_num_arr[j+2]
                idx2 = 16 * seq_num_arr[j+l] + 4 * seq_num_arr[j+l+1] + seq_num_arr[j+l+2]
            else:
                idx1, idx2 = 0, 0
...
@njit(cache=True, nogil=True, fastmath=True, parallel=True)
def numba_calc_cc(seq_num_arr, prop1_arr, prop2_arr, kmer, lag_max):
    seq_len = seq_num_arr.shape[0]
    kmer_num = seq_len - kmer + 1
