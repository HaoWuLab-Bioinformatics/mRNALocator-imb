...
@njit(cache=True, nogil=True, fastmath=True)
def calc_nac_proportion(seq_num_arr: np.ndarray, seq_len: int) -> np.ndarray:
    counts = np.zeros(4, dtype=np.float64)
    for i in prange(seq_len):
        counts[seq_num_arr[i]] += 1
    counts /= seq_len
    return counts

def NAC(sequences, path, dataset_type: str = 'train'):
    NA = 'ACGT'
    encodings = []
    header = ['SampleName']
    header += NA
    ...
    i = -1
    print(f"making {dataset_type.upper()} NAC feature...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        seq_num_arr = dna_str2num_arr(sequence)
        seq_len = len(sequence)
        nac_vals = calc_nac_proportion(seq_num_arr, seq_len)
        code.extend(nac_vals.tolist())
