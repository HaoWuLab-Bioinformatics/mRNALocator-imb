...
def numba_calc_geary(seq_num_arr: np.ndarray, prop_arr: np.ndarray, nlag: int) -> np.ndarray:
    seq_len = seq_num_arr.shape[0]
    N = seq_len - 1
    geary_vals = np.zeros(nlag, dtype=np.float64)
    if N <= 0:
        return geary_vals
    pmean = 0.0
    for j in range(N):
        kmer_idx = 4 * seq_num_arr[j] + seq_num_arr[j+1]
        pmean += prop_arr[kmer_idx]
    pmean /= N
    for d in prange(1, nlag + 1):
        ...
        if Cddown == 0:
            ...
            geary_vals[d-1] = (N / valid_j) * (Cdup / Cddown)
    return geary_vals

def Geary(sequences, nlag, path, dataset_type: str = 'train'):
    ...
    i = -1
    print(f"making {dataset_type.upper()} Geary feature (lag={nlag})...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        seq_num_arr = dna_str2num_arr(sequence)
        for p_name in property_name:
            geary_vals = numba_calc_geary(seq_num_arr, prop_arr_dict[p_name], nlag)
            code.extend(geary_vals.tolist())
...
def process_feature(seq_list, dataset_type, method, feature_method, **kwargs):
    ...
    path = f'{cache_dir}{method}_lag={nlag}.csv'
    if os.path.exists(path):
        print(f"Loading {dataset_type.upper()} {method.upper()} feature (lag={nlag}) from cache...")
...
method = 'Geary'
feature_method = Geary

for nlag in range(2, 5):
    print(f"\nProcessing {method.upper()} with lag={nlag}")
    ...
    print(f"Train {method} feature (lag={nlag}) shape: {train_feature.shape}")
    print(f"Test {method} feature (lag={nlag}) shape: {test_feature.shape}")
    print(f"Weight {method} feature (lag={nlag}) shape: {weight_feature.shape}")

print(f"\n{method.upper()} feature extraction (lag=2~4) done.")