...
@njit(cache=True, nogil=True, fastmath=True)
def numba_calc_tnc(seq_num_arr: np.ndarray) -> np.ndarray:
    seq_len = seq_num_arr.shape[0]
    tmpCode = np.zeros(64, dtype=np.float64)
    for j in range(seq_len - 3 + 1):
        idx = seq_num_arr[j] * 16 + seq_num_arr[j+1] * 4 + seq_num_arr[j+2]
        tmpCode[idx] += 1
    total = tmpCode.sum()
    if total != 0:
        tmpCode = tmpCode / total
    return tmpCode

def TNC(sequences, path, dataset_type: str = 'train'):
    AA = 'ACGT'
    encodings = []
    ...
    i = -1
    print(f"making {dataset_type.upper()} TNC feature...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        seq_num_arr = dna_str2num_arr(sequence)
        tnc_vals = numba_calc_tnc(seq_num_arr)
        code.extend(tnc_vals.tolist())
