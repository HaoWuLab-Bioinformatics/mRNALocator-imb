...
@njit(cache=True, nogil=True, fastmath=True)
def numba_calc_dpcp_base(seq_num_arr: np.ndarray) -> np.ndarray:
    seq_len = seq_num_arr.shape[0]
    dnc_vals = np.zeros(16, dtype=np.float64)
    total = 0
    for j in prange(seq_len - 1):
        pair_idx = 4 * seq_num_arr[j] + seq_num_arr[j+1]
        dnc_vals[pair_idx] += 1
        total += 1
    if total != 0:
        dnc_vals = dnc_vals / total
    return dnc_vals

def DPCP(sequences, path, dataset_type: str = 'train'):
    sequence_type = 'DNA'
    property_name = ['Rise', 'Roll', 'Shift', 'Slide', 'Tilt', 'Twist']
    ...
    header = ['SampleName'] + dinucleotides
    encodings.append(header)

    prop_array = np.array([property_dict[p][:16] for p in property_name], dtype=np.float64)

    i = -1
    print(f"making {dataset_type.upper()} DPCP feature...")
    for sequence in tqdm(sequences):
        i += 1
        code = [f"seq{i}"]
        seq_num_arr = dna_str2num_arr(sequence)
        dnc_vals = numba_calc_dpcp_base(seq_num_arr)
        
        for p_idx in range(len(property_name)):
            normalized_code = dnc_vals * prop_array[p_idx]
