...
def numba_get_kmer_frequency(seq_num_arr: np.ndarray) -> np.ndarray:
    ...
    kmer = 2
    freq = np.zeros(16, dtype=np.float64)
    total = seq_len - kmer + 1
    if total <= 0:
        return freq
    for i in range(total):
        idx = 4 * seq_num_arr[i] + seq_num_arr[i+1]
        freq[idx] += 1
    freq /= total
    return freq
...
def PCPseDNC(sequences, path, dataset_type: str = 'train', lamadaValue: int = 10, weight: float = 0.1):
...
print("\nPCPseDNC feature extraction done.")
